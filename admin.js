...
function renderTable() {
  ...
  projects.forEach((proj, index) => {
    ...
    tr.innerHTML = `
      ...
      <td><input type="text" value="${proj.video1 || ''}" data-index="${index}" data-field="video1"></td>
      <td>
  ...
  // Update projects array on input change
  ...
  try {
    const response = await fetch("/.netlify/functions/updateProjects", {
      ...
      body: JSON.stringify({
        ...
        newJson: { Projects: projects } // wrap array in "Projects"
      }),
    });
